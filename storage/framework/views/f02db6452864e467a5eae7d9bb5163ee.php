...
<div class="container-fluid">
   <div class="card-header pb-0 p-3 d-flex justify-content-between align-items-center mb-4">
    ...
    <button type="button" class="btn btn-primary btn-lg px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#templateModal">
    ...
    <div class="row g-4" id="templates-container">
        ...
        <div class="col-lg-6 col-xl-4" id="template-card-<?php echo e($template->id); ?>">
            <div class="card template-card h-100 shadow-sm border-0">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-2 fw-bold text-dark"><?php echo e($template->name); ?></h5>
                            <span class="badge <?php echo e($template->type === 'email' ? 'badge-email' : 'badge-sms'); ?> fs-6 px-3 py-2 rounded-pill">
                                <i class="mdi <?php echo e($template->type === 'email' ? 'mdi-email' : 'mdi-message-text'); ?> me-1"></i>
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end shadow border-0 py-2">
                                <a href="javascript:void(0);" class="dropdown-item edit-template px-3 py-2" 
                                ...
                                <a href="javascript:void(0);" class="dropdown-item toggle-status px-3 py-2" 
                                   data-id="<?php echo e($template->id); ?>">
                                    <i class="mdi <?php echo e($template->is_active ? 'mdi-eye-off me-2 text-warning' : 'mdi-eye me-2 text-success'); ?>"></i>
                                    <span class="fw-medium"><?php echo e($template->is_active ? 'Deactivate' : 'Activate'); ?></span>
                                </a>
                                <hr class="dropdown-divider my-2">
                                <a href="javascript:void(0);" class="dropdown-item delete-template px-3 py-2 text-danger" 
                                   data-id="<?php echo e($template->id); ?>">
                    ...
                    <?php if($template->type === 'email' && $template->subject): ?>
                    <div class="mb-3 p-3 bg-light rounded">
                        <div class="d-flex align-items-center mb-1">
                            <i class="mdi mdi-email-outline me-2 text-primary"></i>
                            <small class="text-muted fw-medium">SUBJECT</small>
                        </div>
                        <p class="mb-0 fw-medium text-dark"><?php echo e(Str::limit($template->subject, 50)); ?></p>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="mb-4">
                        ...
                        <p class="mb-0 text-dark lh-base"><?php echo e(Str::limit($template->content, 120)); ?></p>
                    </div>

                    <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                        <div>
                            <span class="badge <?php echo e($template->is_active ? 'badge-success-soft' : 'badge-warning-soft'); ?> px-3 py-2 rounded-pill">
                                <i class="mdi <?php echo e($template->is_active ? 'mdi-check-circle' : 'mdi-clock'); ?> me-1"></i>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center py-5">
                    <div class="empty-state">
                        ...
                        <button type="button" class="btn btn-primary btn-lg px-5 shadow-sm" data-bs-toggle="modal" data-bs-target="#templateModal">
            ...
            <form id="templateForm">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control form-control-lg" id="template_name" name="name" required>
                                <div class="invalid-feedback" id="name-error"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="template_type" class="form-label fw-bold">
                                    <i class="mdi mdi-format-list-bulleted me-1 text-primary"></i>Type 
                                    <span class="text-danger">*</span>
                                </label>
                                <select class="form-select form-select-lg" id="template_type" name="type" required>
                    ...
                    <div class="mb-3" id="subject-field" style="display: none;">
                        ...
                        <input type="text" class="form-control form-control-lg" id="template_subject" name="subject">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="template_content" name="content" rows="8" required 
...
$(document).ready(function() {
    ...
    $('#template_type').change(function() {
        ...
        if (type === 'email') {
            $('#subject-field').show();
            $('#template_subject').attr('required', true);
        } else {
            $('#subject-field').hide();
            $('#template_subject').attr('required', false);
        }
    });
    ...
    $('#templateForm').submit(function(e) {
        ...
        const url = isEdit ? "<?php echo e(route('agent.templates.update', ':id')); ?>".replace(':id', $('#template_id').val()) : "<?php echo e(route('agent.templates.store')); ?>";
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                Object.keys(errors).forEach(field => {
                    $(`#template_${field}`).addClass('is-invalid');
            ...
            complete: function() {
                ...
                $('#submit-text').text($('#template_id').val() ? 'Update Template' : 'Save Template');
    ...
    $(document).on('click', '.edit-template', function() {
        ...
        $('#template_type').val(data.type).trigger('change');
        $('#template_subject').val(data.subject);
        $('#template_content').val(data.content);
        
        $('#templateModalLabel').html('<i class="mdi mdi-pencil me-2"></i>Edit Template');
    ...
    $(document).on('click', '.delete-template', function() {
        ...
        if (confirm('🗑️ Are you sure you want to delete this template?\n\nThis action cannot be undone.')) {
            $.ajax({
                url: "<?php echo e(route('agent.templates.destroy', ':id')); ?>".replace(':id', templateId),
                method: 'DELETE',
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        showAlert('success', response.message);
                        $(`#template-card-${templateId}`).fadeOut(300, function() {
                            $(this).remove();
                        });
                    }
                },
    ...
    $(document).on('click', '.toggle-status', function() {
        ...
        $.ajax({
            url: "<?php echo e(route('agent.templates.toggle-status', ':id')); ?>".replace(':id', templateId),
            method: 'PATCH',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content')
            },
    ...
    $('#templateModal').on('hidden.bs.modal', function() {
        ...
        $('#template_id').val('');
        $('#templateModalLabel').html('<i class="mdi mdi-plus-circle me-2"></i>Add New Template');
...
// Function to get templates for dropdown (use this in your forms)
function loadTemplates(type, selectElementId) {
    $.ajax({
        url: '/templates/api/get-templates',
        method: 'GET',
        data: { type: type },
        success: function(response) {
            if (response.success) {
                ...
                response.templates.forEach(template => {
                    select.append(`<option value="${template.id}" 
                        data-content="${template.content}" 
                        data-subject="${template.subject || ''}">${template.name}</option>`);
                });
            }
        }
    });
...
.modal-backdrop.show {
    opacity: 2 !important;  /* very light black, adjust 0.1–0.2 as you like */
  
}


.modal-backdrop.show {
    background-color: rgba(0, 0, 0, 0.15) !important;  /* light black with 15% opacity */
}
...
<?php echo $__env->make('layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\ADMIN\Desktop\listr\resources\views/agents/master-list/index.blade.php ENDPATH**/ ?>