...
<div class="container-fluid">
    ...
    <div class="row g-4" id="trade-persons-container">
        @forelse($tradePersons as $tradePerson)
        <div class="col-lg-6 col-xl-4" id="trade-person-card-{{ $tradePerson->id }}">
            <div class="card trade-person-card h-100 shadow-sm border-0">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-2 fw-bold text-dark">{{ $tradePerson->name }}</h5>
                            <span class="badge badge-profession fs-6 px-3 py-2 rounded-pill">
                                <i class="mdi mdi-hammer-wrench me-1"></i>
                                {{ $tradePerson->profession }}
                            </span>
                        </div>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end shadow border-0 py-2">
                                <a href="javascript:void(0);" class="dropdown-item edit-trade-person px-3 py-2" 
                                   data-id="{{ $tradePerson->id }}"
                                   data-name="{{ $tradePerson->name }}"
                                   data-profession="{{ $tradePerson->profession }}"
                                   data-email="{{ $tradePerson->email }}"
                                   data-phone="{{ $tradePerson->phone }}">
                                ...
                                <a href="javascript:void(0);" class="dropdown-item toggle-status px-3 py-2" 
                                   data-id="{{ $tradePerson->id }}">
                                    <i class="mdi {{ $tradePerson->is_active ? 'mdi-eye-off me-2 text-warning' : 'mdi-eye me-2 text-success' }}"></i>
                                    <span class="fw-medium">{{ $tradePerson->is_active ? 'Deactivate' : 'Activate' }}</span>
                                </a>
                    ...
                    <div class="mb-3 p-3 bg-light rounded">
                        ...
                        <p class="mb-0 fw-medium text-dark">{{ $tradePerson->email }}</p>
                    </div>
                    ...
                    <div class="mb-4 p-3 bg-light rounded">
                        ...
                        <p class="mb-0 fw-medium text-dark">{{ $tradePerson->phone }}</p>
                    </div>

                    <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                        <div>
                            <span class="badge {{ $tradePerson->is_active ? 'badge-success-soft' : 'badge-warning-soft' }} px-3 py-2 rounded-pill">
                                <i class="mdi {{ $tradePerson->is_active ? 'mdi-check-circle' : 'mdi-clock' }} me-1"></i>
                                {{ $tradePerson->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        <small class="text-muted">
                            <i class="mdi mdi-calendar me-1"></i>
                            {{ $tradePerson->created_at->diffForHumans() }}
                        </small>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('#tradePersonForm').submit(function(e) {
        ...
        const url = isEdit ? "{{ route('agent.trade-persons.update', ':id') }}".replace(':id', $('#trade_person_id').val()) : "{{ route('agent.trade-persons.store') }}";
    ...
    $(document).on('click', '.delete-trade-person', function() {
        ...
        if (confirm('🗑️ Are you sure you want to delete this trade person?\n\nThis action cannot be undone.')) {
            $.ajax({
                url: "{{ route('agent.trade-persons.destroy', ':id') }}".replace(':id', tradePersonId),
                method: 'DELETE',
                data: {
    ...
    $(document).on('click', '.toggle-status', function() {
        ...
        $.ajax({
            url: "{{ route('agent.trade-persons.toggle-status', ':id') }}".replace(':id', tradePersonId),
            method: 'PATCH',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content')
            },
