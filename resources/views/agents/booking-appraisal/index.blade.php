                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('agent.booking-appraisals.create') }}" class="btn btn-primary">
...
$(document).ready(function() {
    ...
    const table = $('#appraisals-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('agent.booking-appraisals.index') }}",
            data: function(d) {
   ...
   { 
    ...
    $(document).on('click', '.view-btn', function() {
        ...
        $.ajax({
            url: "{{ route('agent.booking-appraisals.show', '') }}/" + appraisalId,
            type: 'GET',
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('agent.booking-appraisals.destroy', '') }}/" + appraisalId,
                    type: 'DELETE',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
