                        ...
                        <div class="card">
                            <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                                ...
                                <a href="#" class="btn btn-primary" id="show-register-tab">
                                    <i class="fas fa-plus me-1"></i> Register New Agent
                                </a>
                            </div>
                            <div class="card-body">
                                <!-- Tabs Navigation -->
                                <ul class="nav nav-tabs" id="agentTabs" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link {{ $activeTab === 'manage' ? 'active' : '' }}" 
                                ...
                                <div class="tab-content" id="agentTabsContent">
                                    ...
                                    <div class="tab-pane fade {{ $activeTab === 'manage' ? 'show active' : '' }}" 
                                        ...
                                        <div class="table-responsive">
                                            <table id="manage-agents-table" class="table table-hover" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th>Name</th>
                                                        <th>Email</th>
                                    ...
                                    <div class="tab-pane fade {{ $activeTab === 'register' ? 'show active' : '' }}" 
                                        ...
                                        <div class="register-form">
                                            <form id="agent-registration-form" method="POST" action="{{ route('agent.store') }}">
                                                @csrf
                                                <h5 class="mb-4">Register New Agent</h5>
                                    ...
                                    <div class="tab-pane fade {{ $activeTab === 'inactive' ? 'show active' : '' }}" 
                                        ...
                                        <div class="table-responsive">
                                            <table id="inactive-agents-table" class="table table-hover" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th>Name</th>
                                                        <th>Email</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-4">
                    <div class="mb-4">
                        <h6 class="fw-bold mb-3 text-uppercase text-sm text-muted">Agent Information</h6>
                        ...
                        <div class="d-flex">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <span id="agent-status" class="badge"></span>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    tabElms.forEach(tabEl => {
        tabEl.addEventListener('shown.bs.tab', function (event) {
            ...
            if (target === '#manage') {
                manageAgentsTable.ajax.reload();
            } else if (target === '#new') {
                newApplicationsTable.ajax.reload();
            } else if (target === '#inactive') {
                inactiveAgentsTable.ajax.reload();
            }
        });
    });

    // Show register tab when "Register New Agent" button is clicked
    ...
    const manageAgentsTable = $('#manage-agents-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('agent.getAgents') }}",
        columns: [
            { data: 'name', name: 'name' },
    ...
    const newApplicationsTable = $('#new-applications-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('agent.getNewApplications') }}",
        columns: [
            { data: 'name', name: 'name' },
            { data: 'email', name: 'email' },
            { data: 'phone', name: 'phone' },
    ...
    const inactiveAgentsTable = $('#inactive-agents-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('agent.getInactiveAgents') }}",
        columns: [
            { data: 'name', name: 'name' },
            { data: 'email', name: 'email' },
            { data: 'phone', name: 'phone' },
            { 
                ...
                render: function(data, type, row) {
                    let badgeClass = 'badge-warning';
                    if (data === 'active') badgeClass = 'badge-success';
                    if (data === 'in_active' ) badgeClass = 'badge-danger';
        ...
        responsive: true,
        language: {
            emptyTable: 'No inactive agents found',
            info: 'Showing _START_ to _END_ of _TOTAL_ agents',
            infoEmpty: 'Showing 0 to 0 of 0 agents',
            infoFiltered: '(filtered from _MAX_ total agents)',
            lengthMenu: 'Show _MENU_ agents',
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $modal.find('#agent-name').text(agent.name);
                    $modal.find('#agent-email').text(agent.email);
                    $modal.find('#agent-phone').text(agent.phone);
                    
                    // Set status badge
                    let statusClass = 'badge-warning';
                    if (agent.status === 'active') statusClass = 'badge-success';
                    if (agent.status === 'in_active' || agent.status === 'rejected') statusClass = 'badge-danger';
                    $modal.find('#agent-status').text(agent.status).addClass(statusClass);
                    
                    $modal.find('#agent-address1').text(agent.address_line1);
                    $modal.find('#agent-address2').text(agent.address_line2 || 'N/A');
                    $modal.find('#agent-location').text(`${agent.city}, ${agent.state} ${agent.zipcode}`);
                    $modal.find('#agent-created').text(new Date(agent.created_at).toLocaleString());
                    $modal.find('#agent-updated').text(new Date(agent.updated_at).toLocaleString());
                    
                    // Set edit button href
                    $modal.find('#edit-agent-btn').attr('href', `/agent/${agentId}/edit`);
    ...
    $(document).on('change', '.status-select', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
					url:`https://spiderdesk.asia/listr/agent/${agentId}/update-status`,
                    type: 'POST',
                    data: {
                        status: newStatus,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            manageAgentsTable.ajax.reload(null, false);
                            newApplicationsTable.ajax.reload(null, false);
                            inactiveAgentsTable.ajax.reload(null, false);
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('agent.destroy', '') }}/" + agentId,
                    type: 'DELETE',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    $(document).on('click', '.approve-btn', function() {
        ...
        Swal.fire({
            title: 'Approve Agent?',
            text: "This agent will be approved and activated.",
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('agent.approve', '') }}/" + agentId,
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
...
$(document).on('change', '.status-select', function() {
    ...
    $(document).on('click', '.reject-btn', function() {
        ...
        Swal.fire({
            title: 'Reject Agent?',
            text: "This agent application will be rejected.",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('agent.reject', '') }}/" + agentId,
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    $(document).on('click', '.activate-btn', function() {
        ...
        Swal.fire({
            ...
            text: "This agent will be activated and marked as approved.",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#28a745',
            cancelButtonColor: '#6c757d',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('agent.activate', '') }}/" + agentId,
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    $('#agent-registration-form').submit(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: response.message || 'Agent registered successfully',
                    timer: 2000,
                    showConfirmButton: false
                }).then(() => {
                    // Reset form
