...
function getRoutes() {
    if (currentUserType === 'agency') {
        return {
            sendOtp: '{{ route("agency.login.send.otp") }}',
            resendOtp: '{{ route("agency.login.resend.otp") }}',
            verifyOtp: '{{ route("agency.login.verify.otp") }}',
            dashboard: '{{ route("agency.dashboard") }}'
        };
    } else {
        return {
            sendOtp: '{{ route("agent.login.send.otp") }}',
            resendOtp: '{{ route("agent.login.resend.otp") }}',
            verifyOtp: '{{ route("agent.login.verify.otp") }}',
...
async function tryAgencyLogin(formData) {
    try {
        const response = await fetch('{{ route("agency.login.send.otp") }}', {
            method: 'POST',
            body: formData,
            headers: {
...
async function tryAgentLogin(formData) {
    try {
        const response = await fetch('{{ route("agent.login.send.otp") }}', {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
